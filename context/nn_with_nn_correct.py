...
class NeuralNetwork:
    def __init__(self):
        ...
        self.dssr = np.zeros(self.num_inputs)
    ...
    def __backward_propagation(self, x, y, lr):
        ...
        for i in range(len(x)):
            self.dssr[i] = -2.0 * (y[i] - self.predicted[i])
            diff_bias2 += self.dssr[i]
            for j in range(self.num_hidden_nodes):
                diff_bias1[j] += self.dssr[i] * self.weights2[j] * (1 / (1+np.exp(-1.0 * self.hidden_net[j][i])))
                diff_weights1[j] += self.dssr[i] * self.weights2[j] * (1 / (1+np.exp(-1.0 * self.hidden_net[j][i]))) * x[i]
                diff_weights2[j] += self.dssr[i] * self.hidden_activation[j][i]
    ...
    def train(self, x, y, epochs=500):
        for epoch in range(epochs):
            ...
            print("Predicted:", self.predicted, "Dssr:", self.dssr)
            print("Weights1:", self.weights1, "Weights2:", self.weights2, "Bias1:", self.bias1, "Bias2:", self.bias2)
            print("---------------------")
...
neural_network.train(x_train, y_train, epochs=500)
